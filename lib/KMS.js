...
class KMS {
   ...
   * @param {string} CiphertextBlob base64 encoded string
   * @returns {promise} result
   * @throws {Error} secret could not be decrypted
   *
   * @memberOf KMS
   * @description decrypt the data in a synchronous manner
   * http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/KMS.html#decrypt-property
   */
  decryptDataSync(CiphertextBlob) {
    let result;
    this.decryptData(CiphertextBlob)
      .then(({ Plaintext }) => result = Plaintext,
        (err) => {
          result = err;
